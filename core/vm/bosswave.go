...
func bwVerifyEd25519Packed(args []byte, env Environment) []byte {
...
func bwVerifyEd25519(args []byte, env Environment) []byte {
	vk := args[0:32]
...
func bwUnpackDOT(args []byte, env Environment) []byte {
...
func bwGetDOTDelegatedRevoker(args []byte, env Environment) []byte {
...
func bwUnpackEntity(args []byte, env Environment) []byte {
...
func bwGetEntityDelegatedRevoker(args []byte, env Environment) []byte {
...
func bwUnpackAccessDChain(args []byte, env Environment) []byte {
...
func bwGetDChainDOTHash(args []byte, env Environment) []byte {
...
func bwSliceByte32(args []byte, env Environment) []byte {
	blob := getBytesParam(args, 0)
...
func wrappedBWChainGrants(dc *objects.DChain, adpspacked []byte, mvk []byte, suffix []byte, env Environment) int {
...
func bwADChainGrants(args []byte, env Environment) []byte {
...
func bwGetDOTNumRevokableHashes(args []byte, env Environment) []byte {
...
func bwGetDOTRevokableHash(args []byte, env Environment) []byte {
	dhash := getBytes32Param(args, 0)
	indx := getIntParam(args, 1)
	dot := env.Scratch().LookupSlice(dhash).(*objects.DOT)
...
func bwGetDChainNumRevokableHashes(args []byte, env Environment) []byte {
	chainhash := getBytes32Param(args, 0)
	dc := env.Scratch().LookupSlice(chainhash).(*objects.DChain)
...
func bwGetDChainRevokableHash(args []byte, env Environment) []byte {
	chainhash := getBytes32Param(args, 0)
	indx := getIntParam(args, 1)
	dc := env.Scratch().LookupSlice(chainhash).(*objects.DChain)
	bwHelperAugmentDC(dc, env.Scratch())
...
func bosswave(in []byte, env Environment) (rv []byte) {
	defer func() {
		if r := recover(); r != nil {
